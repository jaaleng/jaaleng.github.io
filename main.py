...
# 这里的 jaaleng.github.io 是您的配置
MD_HEAD = """## [Jaaleng"blog](https://jaaleng.github.io/)
...
def add_issue_info(issue, file_handler): # <--- 修正: 使用 file_handler 提高通用性
    time = format_time(issue.created_at)
    file_handler.write(f"- [{issue.title}]({issue.html_url})--{time}\n")


def add_md_todo(repo, md_filename, me): # <--- 修正: 使用 md_filename 避免冲突
    todo_issues = list(get_todo_issues(repo))
    if not TODO_ISSUES_LABELS or not todo_issues:
        return
    
    with open(md_filename, "a+", encoding="utf-8") as f: # <--- 修正: 使用 f
        f.write("## TODO\n") 
        for issue in todo_issues:
            if is_me(issue, me):
                todo_title, todo_list = parse_TODO(issue)
                f.write("TODO list from " + todo_title + "\n") 
                for t in todo_list:
                    f.write(t + "\n") 
                # new line
                f.write("\n") 
...
def add_md_top(repo, md_filename, me): # <--- 修正: 使用 md_filename 避免冲突
    ...
    with open(md_filename, "a+", encoding="utf-8") as f: # <--- 修正: 使用 f
        ...
        for issue in top_issues:
            if is_me(issue, me):
                add_issue_info(issue, f) # <--- 修正: 传递 f


def add_md_firends(repo, md_filename, me): # <--- 修正: 使用 md_filename 避免冲突
    ...
    with open(md_filename, "a+", encoding="utf-8") as f: # <--- 修正: 使用 f
        ...
        f.write("<details><summary>显示</summary>\n") 
        f.write(s) 
        f.write("</details>\n") 
        f.write("\n\n") 


def add_md_recent(repo, md_filename, me, limit=5): # <--- 修正: 使用 md_filename 避免冲突
    count = 0
...
def add_md_header(md_filename, repo_name): # <--- 修正: 使用 md_filename 避免冲突
    with open(md_filename, "w", encoding="utf-8") as f: # <--- 修正: 使用 f
        f.write(MD_HEAD.format(repo_name=repo_name)) 
        f.write("\n") 


def add_md_label(repo, md_filename, me): # <--- 修正: 使用 md_filename 避免冲突
    ...
    with open(md_filename, "a+", encoding="utf-8") as f: # <--- 修正: 使用 f
        for label in labels:
            ...
            issues = get_issues_from_label(repo,
def is_me(issue, me):
    return issue.user.login == me
...
def add_issue_info(issue, f): # <--- 修正: 使用 f (文件句柄)
    time = format_time(issue.created_at)
    f.write(f"- [{issue.title}]({issue.html_url})--{time}\n") # <--- 修正: 使用 f.write
...
def add_md_todo(repo, md, me):
    ...
    with open(md, "a+", encoding="utf-8") as f: # <--- 修正: 使用 f
        f.write("## TODO\n") # <--- 修正: 使用 f.write
        for issue in todo_issues:
            if is_me(issue, me):
                todo_title, todo_list = parse_TODO(issue)
...
def add_md_header(md, repo_name):
    with open(md, "w", encoding="utf-8") as f: # <--- 修正: 使用 f
        f.write(MD_HEAD.format(repo_name=repo_name)) # <--- 修正: 使用 f.write
        f.write("\n") # <--- 修正: 使用 f.write
...
def add_md_label(repo, md, me):
    ...
    with open(md, "a+", encoding="utf-8") as f: # <--- 修正: 使用 f
        for label in labels:
            ...
            # 添加原作者的逻辑：列表化、排序并检查是否有内容，以确保标题写入
            issues = list(sorted(issues,    "描述": "",
}
